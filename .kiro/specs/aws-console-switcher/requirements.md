...
### v6 整合性修正（基本設計との同期・表記整合）

基本設計（design.md）／リサーチ（research.md）で確定した決定に合わせ、本書の以下の記述を整合させた（要件の実質変更ではなく表記の同期）。

| ID | 区分 | 整合内容 |
| --- | --- | --- |
| A-1 | manifest 権限 | 4.1 の `"alarms"` 用途を「TOTP 待機制御」から「`dom_timeout` の計測＋SW 再起動後のフロー回復（TOTP 待機はホスト側で不使用）」へ整合（design.md dom_timeout 機構） |
| A-2 | MV3 状態管理 | 2.2 のフロー状態管理を `chrome.storage.local` の `FlowContext`（`tabId` キー）による永続化へ整合し、SW 再起動耐性を明記（research.md Implications／design.md「フロー状態の復元」） |
| A-3 | アイドルロック既定 | 4.1.1 のアイドル自動ロック既定値を「20 分（目安 15〜30 分、設定可能）」へ明確化（design.md `ExtensionSettings.idleLockMinutes`） |
...
- フロー状態は SW のメモリに依存させず、`chrome.storage.local` の `FlowContext`（`tabId` をキー）に永続化して SW 再起動耐性を持たせる（基本設計で `FlowContext` として精緻化）。Content Script はページ存続中の DOM 検知を担い、URL パターンで対象ページを識別する。
...
- 権限の最小化（manifest 暫定リスト, M-9）:
  - `permissions`: `"nativeMessaging"`（第一候補の必須）、`"storage"`（UUID・表示用メタデータのローカルキャッシュ。`chrome.storage.local` の利用に必須。2.2/3.1/3.3/3.4 参照）、`"tabs"`（タブ生成/URL 取得/ナビゲーション/前面化、`console.aws.amazon.com` の対象タブ判定）、`"scripting"`（signin 系への Content Script 注入に加え、静的 content_scripts 対象外の `console.aws.amazon.com` タブへ `chrome.scripting.executeScript` で状態検出スクリプトを動的注入する用途。3.1 陳腐化対策参照）、`"alarms"`（`dom_timeout` の計測と SW 再起動後のフロー回復に使用。TOTP 待機はホスト側のため不使用）。※`"cookies"` は案A 専用のため案B 決定により不要。
  - `host_permissions`:
...
- アンロックの実行主体とタイミング（**方式1＋アイドル自動ロックに決定, D-3**）:
  ...
  - アイドル自動ロック: ネイティブホストは最終利用時刻を追跡し、一定アイドル（既定 20 分、目安 15〜30 分の範囲で設定可能）で自動的に `bw lock` を呼ぶ（`bw` CLI セッションは自動失効しないため、ホスト側タイマー実装が必須）。
...
| D-3 | unlock 方式 | **決定済み** | 方式1＋アイドル自動ロック（既定 20 分、目安 15〜30 分で設定可能）。ホスト側でアイドルタイマーを実装し `bw lock` を呼ぶ |
