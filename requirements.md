# 要件定義書：AWS Console Multi-Account Switcher（改訂版 v5）
...
- 状態表示: 現在ログイン中のアカウントを視覚的に区別する。セッション状態の把握方針は次のとおり（設計メモ, M-7 関連）。
  ...
  - 陳腐化対策: `chrome.storage.local` は永続だが**実態と乖離しうる**（サーバ側セッション失効・手動ログアウト・拡張外ログインを反映できない）。`console.aws.amazon.com`（4.1 host_permissions の検出用途）のタブが開いている場合は当該 DOM から現ログイン状態を検出し、記録を補正する。確証が得られない状態は「不確定」として控えめに表示し、誤った「ログイン済み」表示を避ける。
...
5. Step 3（MFA）: MFA 入力画面の DOM 描画を検知し、ネイティブホストから取得した TOTP コードを注入して Submit する。**TOTP の 30 秒ローテーションと AWS 側のコード再利用拒否を考慮し、有効残秒数が不足する場合は次コードを待機・再試行する制御を備えること。** 「不足」の閾値は既定で**残り 5 秒未満**とし、5〜10 秒の範囲で設定可能とする（ネットワーク遅延・サーバ時刻ずれによる失効を回避）。具体値は PoC #4 で最終確認する。
...
| TOTP シード | 標準 Authenticator key (`login.totp`) | ✓ | Step 3 のコード生成 |
...
- UUID キャッシュの再同期トリガー:
  - (a) `bw get totp/item <UUID>` または `bw get item <UUID>` が失敗した場合（UUID 無効化・アイテム削除など）
  - (b) ユーザーが手動で「同期」操作を実行した場合
  - (c) Vault ロック解除後の初回アクセス時（推奨）
  - 再同期時は `bw list items --folderid <id>` で再列挙し、キャッシュを上書き更新する。
  - 失敗した UUID は即座に無効化し、再同期完了までそのアカウントへの自動ログインを停止してユーザーへ通知する。
...
| (b) AWS 認証エラー | パスワード誤り、TOTP 拒否（再利用/時刻ずれ）、アカウントロックアウト、対象に MFA 未設定で MFA 画面が出ない | TOTP は次コードで 1 回まで自動再試行。上限超過・認証エラーはフローを停止しユーザーに通知 |
...
- 権限の最小化（manifest 暫定リスト, M-9）:
  - `permissions`: `"nativeMessaging"`（第一候補の必須）、`"storage"`（UUID・表示用メタデータのローカルキャッシュ。`chrome.storage.local` の利用に必須。2.2/3.1/3.3/3.4 参照）、`"tabs"`（タブ生成/URL 取得/ナビゲーション/前面化）、`"scripting"`（Content Script 注入）、`"alarms"`（TOTP 待機制御に使う場合）。※`"cookies"` は案A 専用のため案B 決定により不要。
  - `host_permissions`:
