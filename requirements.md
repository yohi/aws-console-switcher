...
5. Step 3（MFA）: MFA 入力画面の DOM 描画を検知し、ネイティブホストから取得した TOTP コードを注入して Submit する。**TOTP の 30 秒ローテーションと AWS 側のコード再利用拒否を考慮し、有効残秒数が不足する場合は次コードを待機・再試行する制御を備えること。**
...
- 権限の最小化（manifest 暫定リスト, M-9）:
  - `permissions`: `"nativeMessaging"`（第一候補の必須）、`"tabs"`（タブ生成/URL 取得/ナビゲーション/前面化）、`"scripting"`（Content Script 注入）、`"alarms"`（TOTP 待機制御に使う場合）。※`"cookies"` は案A 専用のため案B 決定により不要。
  - `host_permissions`:
...
- アンロックの実行主体とタイミング（**方式1＋アイドル自動ロックに決定, D-3**）:
  - 起動時 1 回アンロック: OS ログインまたはホスト起動時に 1 回 `bw unlock`（マスターパスワード入力は起動時の 1 回）。能動利用中はゼロタッチで動作する。
